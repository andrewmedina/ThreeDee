...
void process_assignment(ASTNode *statement)
{
    ...
    ASTNode *operator= identifier->right->right->left->right;
    ASTNode *operand = operator->right;
    ...
    assigned_var->value = do_math(assigned_var->value, operator->value, atoi(operand->value));
...
void generate_gcode(ASTNode *node)
{
    ...
    case AST_PRINT:
    {
        // Print the value of a variable as Gcode
